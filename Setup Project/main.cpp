...
int main(int argc, char **argv)
{
	...
	window = SDL_CreateWindow("666", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, SDL_WINDOW_SHOWN /* | SDL_WINDOW_FILLSCREEN*/);
	...
	destinationRectangle.w = sourceRectangle.w*0.5; //copy the width of our texture
	destinationRectangle.h = sourceRectangle.h*0.5; //copy the height of our texture
	...
	while (loop){
		...
		//set drawing colour for the renderer, number are: RBGA(alpha, transparency). All value between 0-255
		SDL_SetRenderDrawColor(renderer, 255, 0, 168, 255);
		//clear screen with current draw colour
		SDL_RenderClear(renderer);
		//TODO draw stuff to renderer here, like game images, colour, ui whatever
		...
		//render rectangle to screen (renderer and rectangle for positioning)
		SDL_RenderFillRect(renderer, &rect);

		//DRAW OUR 2D IMAGE HERE
